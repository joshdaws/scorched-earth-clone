...
- `npm run release:handoff` reports repo-side handoff artifacts are present,
  ...
  summary, confirms screenshot dimensions/index/per-capture console state,
  confirms those receipts are not stale against covered screen, visual/runtime,
  and campaign layout/progression files, confirms runtime manifest assets exist
  without temporary placeholder naming, confirms `www` is fresh against source
  JS/config/static assets, confirms fresh browser
